...
class HERv2Adapter(BaseBackend):
    ...
    def prepare_fixed_turn_input(
        ...
        *,
        ...
        removed_sections = snapshot.get("removed_section_keys", [])
        if not isinstance(removed_sections, (list, tuple)) or any(
            not isinstance(key, str) or not key.strip() for key in removed_sections
        ):
            raise HerFixedProtocolError(
                "invalid_pcm_snapshot",
                "PCM removed_section_keys must be a sequence of non-empty strings.",
            )
        ...
        encoded, audit = self._session_coordinator.prepare_transport(
            ...
            **binding,
            removed_section_keys=removed_sections,
    ...
    def interrupt_nowait(self, reason: str = "USER_STOP") -> int:
        """Interrupt active HER turns without depending on their asyncio loop."""
        ...
        try:
            ...
            active = tuple(
                runtime
                for key in tuple(self._active_runtimes)
                if (runtime := self._active_runtimes.get(key)) is not None
            )
        interrupted = 0
        for runtime in active:
            interrupt = getattr(runtime, "interrupt_nowait", None)
            if callable(interrupt):
                interrupted += int(interrupt(reason) or 0)
        return interrupted
