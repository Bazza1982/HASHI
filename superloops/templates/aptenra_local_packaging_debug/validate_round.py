...
EXPECTED_PFJ_IDS = [f"PFJ-{index:03d}" for index in range(1, 36)]
EXPECTED_QUESTION = (
...
def validate_round(loop_dir: Path, phase: str) -> dict[str, Any]:
    ...
    if required_ids != EXPECTED_PFJ_IDS:
        error("registry_required_ids", "Registry must list PFJ-001 through PFJ-035 exactly once and in order.")
    ...
    if failure_ids != EXPECTED_PFJ_IDS:
        error("registry_entries", "Registry entries must cover PFJ-001 through PFJ-035 exactly once and in order.")
