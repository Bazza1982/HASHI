...
impl Session {
    ...
    /// Preserve the complete persisted transcript before active model context
    /// is replaced by a semantic summary. The archive is intentionally
    /// append-only from the runtime's perspective: compaction never deletes or
    /// rotates these recoverable source records.
    pub fn archive_before_compaction(&self) -> Result<Option<PathBuf>, SessionError> {
        ...
        let archive_name = format!(
            "{file_name}.pre-compaction-{next_count:04}-{}.jsonl",
            self.updated_at_ms
        );
...
fn generate_session_id() -> String {
    ...
    format_session_id(millis, std::process::id(), counter)
}

fn format_session_id(millis: u64, process_id: u32, counter: u64) -> String {
    // Multiple short-lived CLI processes can start in the same millisecond.
    // Their process-local counters all begin at zero, so the PID is required
    // to keep independently-created managed sessions from sharing a path.
...
fn temporary_path_for_components(
    path: &Path,
    millis: u64,
    process_id: u32,
    counter: u64,
    ...
    let file_name = path
        ...
        .and_then(|value| value.to_str())
        .unwrap_or("session");
    path.with_file_name(format!("{file_name}.tmp-{millis}-{process_id}-{counter}"))
}
...
mod tests {
    use super::{
        cleanup_rotated_logs, current_time_millis, format_session_id,
        parse_created_at_ms_from_session_id, rotate_session_file_if_needed,
        temporary_path_for_components, ContentBlock, ConversationMessage, MessageRole, Session,
        SessionFork,
    };
    ...
    fn created_at_parser_requires_full_session_id_shape() {
        ...
        assert_eq!(
            parse_created_at_ms_from_session_id("session-1743724800123-4242-0"),
            Some(1_743_724_800_123)
        );
    }

    #[test]
    fn session_and_atomic_temp_names_are_unique_across_processes() {
        let millis = 1_743_724_800_123;
        let first_pid = 41;
        let second_pid = 42;

        let first_session = format_session_id(millis, first_pid, 0);
        let second_session = format_session_id(millis, second_pid, 0);
        assert_ne!(first_session, second_session);

        let target = Path::new("session.jsonl");
        let first_temp = temporary_path_for_components(target, millis, first_pid, 1);
        let second_temp = temporary_path_for_components(target, millis, second_pid, 1);
