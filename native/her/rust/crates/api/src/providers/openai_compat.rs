...
impl StreamState {
    ...
    fn ingest_chunk(&mut self, chunk: ChatCompletionChunk) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        for choice in chunk.choices {
            for reasoning in choice.delta.reasoning_fragments() {
                ...
                events.push(StreamEvent::ContentBlockDelta(ContentBlockDeltaEvent {
                    ...
                    delta: ContentBlockDelta::ThinkingDelta {
                        ...
                        source: Some(reasoning.source),
                        visibility: Some(reasoning.visibility),
                    },
                }));
            }
...
impl ChunkDelta {
    fn reasoning_fragments(&self) -> Vec<ReasoningFragment> {
        reasoning_fragments(
            [
            ...
            self.reasoning_details.as_slice(),
        )
    }
}
...
fn build_chat_completion_request_for_base_url(
    ...
    messages = group_tool_media_after_tool_results(messages);
...
fn is_tool_media_message(message: &Value) -> bool {
    message.get("role").and_then(Value::as_str) == Some("user")
        && message
            ...
            .is_some_and(|text| text.starts_with("Visual media returned by tool call "))
}

/// Keep every OpenAI tool response contiguous with its assistant tool-call
/// message. Multimodal user messages derived from those responses are emitted
/// immediately after the complete run of tool responses, which preserves the
/// provider pairing invariant when an assistant requests parallel tools.
fn group_tool_media_after_tool_results(messages: Vec<Value>) -> Vec<Value> {
    let mut grouped = Vec::with_capacity(messages.len());
    let mut pending_media = Vec::new();
    for message in messages {
        if is_tool_media_message(&message) {
            pending_media.push(message);
            continue;
        }
        if message.get("role").and_then(Value::as_str) != Some("tool") && !pending_media.is_empty()
        {
            grouped.append(&mut pending_media);
        }
    ...
    match message.role.as_str() {
        ...
        _ => message
            ...
            .flat_map(|block| match block {
                InputContentBlock::Text { text } => vec![json!({
                    "role": "user",
                    "content": text,
                })],
                InputContentBlock::ToolResult {
                    ...
                    let images = content
                        ...
                        .filter_map(|item| match item {
                            ...
                            | ToolResultContentBlock::Json { .. } => None,
                        })
                        .collect::<Vec<_>>();
                    if !images.is_empty() {
                        let mut parts = vec![json!({
                            "type": "text",
                            "text": format!(
                                "Visual media returned by tool call {tool_use_id} follows."
                            ),
                        })];
                        parts.extend(images);
                        messages.push(json!({
    ...
    for (i, block) in content.iter().enumerate() {
        ...
        match block {
            ...
            ToolResultContentBlock::Image {
                ...
                result.push_str(" image attached separately]");
            }
        }
    }
    result
...
fn normalize_response(
    ...
    if let Some(thinking) = first_non_empty_reasoning(
        [
            (
                "reasoning_content",
                choice.message.reasoning_content.as_deref(),
            ),
            ("reasoning", choice.message.reasoning.as_deref()),
        ],
        choice.message.reasoning_details.as_slice(),
    ) {
...
fn first_non_empty_reasoning<const N: usize>(
    fields: [(&str, Option<&str>); N],
    details: &[ReasoningDetail],
) -> Option<String> {
    reasoning_fragments(fields, details)
        .into_iter()
        .find(|fragment| fragment.visibility == "provider_returned")
        .map(|fragment| fragment.text)
}

fn reasoning_fragments<const N: usize>(
    fields: [(&str, Option<&str>); N],
    details: &[ReasoningDetail],
) -> Vec<ReasoningFragment> {
    ...
    for (source, value) in fields {
        if let Some(value) = value {
            // A streaming delta's leading/trailing whitespace is semantic.
            // Drop only truly empty values and forward every other byte intact.
            if !value.is_empty() {
    ...
    let has_direct_reasoning_text = !text_chunks.is_empty();

    for detail in details {
        let detail_type = detail.detail_type.as_deref().unwrap_or("");
        let is_encrypted_or_redacted =
        ...
        if let Some(text) = text {
            let source = if detail.text.as_deref().is_some() {
                "reasoning_details.text"
            } else {
                "reasoning_details.summary"
            };
            if is_encrypted_or_redacted {
                redacted.push(ReasoningFragment {
                    text: "provider emitted encrypted reasoning block".to_string(),
                    source: source.to_string(),
                    visibility: "provider_redacted".to_string(),
                });
            } else if !has_direct_reasoning_text {
                text_chunks.push(text.to_string());
                sources.push(source.to_string());
            }
        } else if is_encrypted_or_redacted {
            redacted.push(ReasoningFragment {
                text: "provider emitted encrypted reasoning block".to_string(),
...
mod tests {
    ...
    #[test]
    fn non_streaming_openrouter_reasoning_detail_emits_thinking_block() {
        let response = super::ChatCompletionResponse {
            id: "chatcmpl_reasoning_detail".to_string(),
            ...
            choices: vec![super::ChatChoice {
                message: super::ChatMessage {
                    ...
                    reasoning_content: None,
                    reasoning: None,
                    reasoning_details: vec![super::ReasoningDetail {
                        detail_type: None,
                        text: Some("detail thought".to_string()),
                        summary: None,
                    }],
                    tool_calls: Vec::new(),
                },
        ...
        let normalized =
            normalize_response("deepseek/deepseek-v4-pro", response).expect("normalized");
        assert!(matches!(
            normalized.content.first(),
            Some(OutputContentBlock::Thinking { thinking, .. }) if thinking == "detail thought"
        ));
    }

    #[test]
    fn streaming_reasoning_details_emit_text_and_redacted_metadata() {
        let mut state = StreamState::new("deepseek/deepseek-v4-pro".to_string());
        let events = state
            .ingest_chunk(super::ChatCompletionChunk {
                id: "chatcmpl_reasoning_details".to_string(),
                ...
                choices: vec![super::ChunkChoice {
                    delta: super::ChunkDelta {
                        ...
                        reasoning_details: vec![
                            super::ReasoningDetail {
                                detail_type: Some("reasoning.text".to_string()),
                                text: Some("first detail".to_string()),
                                summary: None,
                            },
                            super::ReasoningDetail {
                                detail_type: Some("reasoning.summary".to_string()),
                                text: None,
                                summary: Some("second detail".to_string()),
                            },
                            super::ReasoningDetail {
                                detail_type: Some("reasoning.encrypted".to_string()),
                                text: None,
                                summary: None,
                            },
                        ],
        ...
        assert!(matches!(
            ...
            StreamEvent::ContentBlockDelta(ContentBlockDeltaEvent {
                ...
                ..
            }) if thinking == "first detail\nsecond detail"
                && source.as_deref() == Some("reasoning_details.text,reasoning_details.summary")
                && visibility.as_deref() == Some("provider_returned")
        ));
        assert!(matches!(
            events[3],
            StreamEvent::ContentBlockDelta(ContentBlockDeltaEvent {
                ...
                ..
            }) if source.as_deref() == Some("reasoning_details.encrypted")
                && visibility.as_deref() == Some("provider_redacted")
        ));
    }

    #[test]
    fn streaming_reasoning_prefers_top_level_text_over_duplicate_details() {
        let mut state = StreamState::new("deepseek/deepseek-v4-pro".to_string());
        let events = state
            .ingest_chunk(super::ChatCompletionChunk {
                id: "chatcmpl_duplicate_reasoning".to_string(),
                model: Some("deepseek/deepseek-v4-pro".to_string()),
                choices: vec![super::ChunkChoice {
                    delta: super::ChunkDelta {
                        content: None,
                        reasoning_content: None,
                        reasoning: Some("top level chunk".to_string()),
                        thinking: None,
                        reasoning_details: vec![super::ReasoningDetail {
                            detail_type: Some("reasoning.text".to_string()),
                            text: Some("top level chunk".to_string()),
                            summary: None,
                        }],
                        tool_calls: Vec::new(),
                    },
            ...
            .expect("duplicate reasoning chunk");
        ...
        assert!(matches!(
            ...
            StreamEvent::ContentBlockDelta(ContentBlockDeltaEvent {
                ...
                ..
            }) if thinking == "top level chunk" && source.as_deref() == Some("reasoning")
        ));
        assert_eq!(
            events
                .iter()
    ...
    #[test]
    fn reasoning_fragments_preserve_exact_stream_whitespace() {
        let mut reconstructed = String::new();
        for piece in ["A", " ", "EST", " and was flag", "ged as miss", "ed"] {
            let fragments = super::reasoning_fragments([("reasoning", Some(piece))], &[]);
            reconstructed.push_str(&fragments[0].text);
        }

        assert_eq!(reconstructed, "A EST and was flagged as missed");

        let details = vec![super::ReasoningDetail {
            detail_type: Some("reasoning.text".to_string()),
            text: Some(" detail with edges ".to_string()),
            summary: None,
        }];
        let fragments = super::reasoning_fragments([("reasoning", None)], details.as_slice());
        assert_eq!(fragments[0].text, " detail with edges ");
    }
    ...
    #[test]
    fn translate_tool_result_emits_images_as_following_user_multimodal_message() {
        use crate::types::{ImageSource, InputContentBlock, InputMessage, ToolResultContentBlock};
        ...
        let message = InputMessage {
            ...
            content: vec![InputContentBlock::ToolResult {
                ...
                content: vec![
                    ToolResultContentBlock::Text {
                        text: "normalized image metadata".to_string(),
                    },
                    ToolResultContentBlock::Image {
                        source: ImageSource::Base64 {
                            media_type: "image/jpeg".to_string(),
                            data: "/9j/2Q==".to_string(),
                        },
                    },
                ],
                is_error: false,
        ...
        let anthropic_shape = serde_json::to_value(&message).expect("message should serialize");
        assert_eq!(
            anthropic_shape["content"][0]["content"][1]["source"]["type"],
            json!("base64")
        );
        assert_eq!(
            anthropic_shape["content"][0]["content"][1]["source"]["media_type"],
            json!("image/jpeg")
        );

        let translated = super::translate_message(&message, "gpt-4o");
        assert_eq!(translated.len(), 2);
        assert_eq!(translated[0]["role"], json!("tool"));
        assert_eq!(translated[0]["tool_call_id"], json!("call-image"));
        assert!(translated[0]["content"]
            .as_str()
            .is_some_and(|text| text.contains("attached separately") && !text.contains("/9j/2Q")));
        assert_eq!(translated[1]["role"], json!("user"));
        assert_eq!(
            translated[1]["content"][1]["image_url"]["url"],
            json!("data:image/jpeg;base64,/9j/2Q==")
        );
    }

    #[test]
    fn request_groups_parallel_tool_results_before_their_media_messages() {
        use crate::types::{
            ImageSource, InputContentBlock, InputMessage, MessageRequest, ToolResultContentBlock,
        };

        let tool_result = |id: &str| InputMessage {
            role: "user".to_string(),
            content: vec![InputContentBlock::ToolResult {
                tool_use_id: id.to_string(),
                content: vec![ToolResultContentBlock::Image {
                    source: ImageSource::Base64 {
                        media_type: "image/jpeg".to_string(),
                        data: "/9j/2Q==".to_string(),
                    },
                ...
                InputMessage {
                    ...
                    content: vec![
                        InputContentBlock::ToolUse {
                            ...
                            input: json!({"path": "a.jpg"}),
                        },
                        InputContentBlock::ToolUse {
                            id: "call-b".to_string(),
                            name: "media_read".to_string(),
                            input: json!({"path": "b.jpg"}),
                        },
                    ],
                },
                tool_result("call-a"),
        ...
        let payload =
            super::build_chat_completion_request(&request, super::OpenAiCompatConfig::openai());
        let roles = payload["messages"]
            .as_array()
            .expect("messages")
            .iter()
