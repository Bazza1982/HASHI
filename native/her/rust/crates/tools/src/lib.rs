...
use runtime::{
    ...
    RuntimeStreamEvent, RuntimeStreamObserver, Session, TaskPacket, ToolError, ToolExecutor,
};
...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ToolSpec {
            name: "bash",
            description: "Execute a shell command in the Claw-local current workspace. This is not the HASHI access_root filesystem authority. Timeout values are seconds.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "command": { "type": "string" },
                    "timeout_seconds": { "type": "integer", "minimum": 1, "description": "Maximum runtime in seconds." },
        ...
        ToolSpec {
            name: "read_file",
            description: "Read a text file from the Claw-local current workspace. This does not read through the HASHI access_root authority; use hashi_file_read for that scope.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string" },
        ...
        ToolSpec {
            name: "write_file",
            description: "Write a text file in the Claw-local current workspace. This does not write through the HASHI access_root authority; use hashi_file_write for that scope.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string" },
        ...
        ToolSpec {
            name: "edit_file",
            description: "Replace text in a file inside the Claw-local current workspace. This is distinct from the HASHI access_root authority.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string" },
        ...
        ToolSpec {
            name: "glob_search",
            description: "Find files by glob pattern inside the Claw-local current workspace. This is distinct from the HASHI access_root authority.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "pattern": { "type": "string" },
        ...
        ToolSpec {
            name: "grep_search",
            description: "Search file contents with a regex pattern inside the Claw-local current workspace. This is distinct from the HASHI access_root authority.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "pattern": { "type": "string" },
        ...
        ToolSpec {
            name: "claw_local_cron_create",
            description: "Create a recurring task in the Claw-local cron store. This does not create a HASHI Scheduler job.",
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolSpec {
            name: "claw_local_cron_delete",
            description: "Delete a recurring task from the Claw-local cron store by ID. This does not delete a HASHI Scheduler job.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "cron_id": { "type": "string" }
        ...
        ToolSpec {
            name: "claw_local_cron_list",
            description: "List recurring tasks from the Claw-local cron store only. An empty result says nothing about HASHI Scheduler jobs.",
            input_schema: json!({
                "type": "object",
                "properties": {},
                "additionalProperties": false
...
fn execute_tool_with_enforcer(
    ...
    match name {
        ...
        "claw_local_cron_create" => from_value::<CronCreateInput>(input).and_then(run_cron_create),
        "claw_local_cron_delete" => from_value::<CronDeleteInput>(input).and_then(run_cron_delete),
        "claw_local_cron_list" => run_cron_list(input.clone()),
...
impl ApiClient for ProviderRuntimeClient {
    fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
        self.stream_observed(request, None)
    }
    ...
    fn stream_observed(
        ...
        let tool_choice =
            (request.allow_tools && !self.allowed_tools.is_empty()).then_some(ToolChoice::Auto);
        ...
        for (index, entry) in chain.iter().enumerate() {
            let message_request = MessageRequest {
                ...
                system: system.clone(),
                tools: (request.allow_tools && !tools.is_empty()).then(|| tools.clone()),
        ...
        match event {
            ...
            ApiStreamEvent::ContentBlockDelta(delta) => match delta.delta {
                ...
                ContentBlockDelta::ThinkingDelta {
                    ...
                    let visibility = visibility.unwrap_or_else(|| "provider_returned".to_string());
            ...
            ApiStreamEvent::MessageStop(_) => {
                saw_stop = true;
                push_observed_event(&mut events, AssistantEvent::MessageStop, &mut observer);
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn exposes_cron_only_under_the_claw_local_authority() {
        let specs = mvp_tool_specs();
        let names = specs.iter().map(|spec| spec.name).collect::<BTreeSet<_>>();

        assert!(names.contains("claw_local_cron_create"));
        assert!(names.contains("claw_local_cron_delete"));
        assert!(names.contains("claw_local_cron_list"));
        ...
        let list = specs
            .iter()
            .find(|spec| spec.name == "claw_local_cron_list")
            .expect("Claw-local cron list spec");
        assert!(list.description.contains("Claw-local cron store"));
        assert!(list.description.contains("HASHI Scheduler"));
    }

    #[test]
    fn native_filesystem_tools_identify_the_claw_local_scope() {
        let specs = mvp_tool_specs();
        for name in [
            "bash",
            "read_file",
            ...
            assert!(
                spec.description.contains("Claw-local"),
                "{name} must identify its Claw-local filesystem scope"
            );
        }
    }
    ...
    fn tool_search_supports_keyword_and_select_queries() {
        ...
        let selected = execute_tool("ToolSearch", &json!({"query": "select:Agent,ToolSearch"}))
            .expect("ToolSearch should succeed");
        ...
        let selected_with_alias = execute_tool(
            "ToolSearch",
            &json!({"query": "select:AgentTool,ToolSearch"}),
        )
        ...
        assert_eq!(selected_with_alias_output["matches"][1], "ToolSearch");

        let removed_skill = execute_tool("ToolSearch", &json!({"query": "select:Skill"}))
            .expect("ToolSearch should ignore unavailable tools");
        let removed_skill_output: serde_json::Value =
            serde_json::from_str(&removed_skill).expect("valid json");
        assert_eq!(removed_skill_output["matches"], json!([]));
    }
