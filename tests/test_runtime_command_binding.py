...
def test_command_binding_names_are_unique():
...
def test_get_flexible_bot_commands_appends_runtime_commands(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda: [
            runtime_command_binding.BotCommand("status", "View agent status"),
            runtime_command_binding.BotCommand("private_sample", "Private sample"),
        ],
    )

    commands = runtime_command_binding.get_flexible_bot_commands(SimpleNamespace())
