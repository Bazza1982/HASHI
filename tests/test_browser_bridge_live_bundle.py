...
from tools.browser_bridge_live_bundle import (
    build_live_bundle,
    write_live_bundle,
)
...
def test_build_live_bundle(tmp_path: Path) -> None:
    ...
    assert bundle["probe_step_ids"] == ["healthcheck", "ping", "active_tab", "get_text", "screenshot"]


def test_write_live_bundle(tmp_path: Path) -> None:
    _write_inputs(tmp_path)
    ...
    saved = json.loads((tmp_path / "state" / "live_bundle.json").read_text(encoding="utf-8"))
    assert saved == bundle
