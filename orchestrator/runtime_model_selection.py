...
def claw_provider_callback_error(runtime, mode_flag: str) -> str | None:
    ...
    if mode_flag == "a" and runtime.config.active_backend != "claw-cli":
        return "Provider control is available only while Claw is active."
...
def set_backend_model(runtime, engine: str, requested: str) -> None:
    normalized = requested.strip() if engine == "claw-cli" else normalize_model(engine, requested)
    if not normalized:
        return
    provider = runtime.get_current_provider() if engine == "claw-cli" else None
    backend_cfg = runtime._get_backend_cfg(engine, provider)
    if backend_cfg is not None and engine != "claw-cli":
...
async def cmd_provider(runtime, update, context: Any) -> None:
    if not runtime._is_authorized_user(update.effective_user.id):
        return
    if runtime.config.active_backend != "claw-cli":
...
async def cmd_model(runtime, update, context: Any) -> None:
    ...
    if runtime.config.active_backend == "claw-cli" and not provider:
        await runtime._reply_text(
            update,
            setting_card(
                "🧠",
                ...
                consequence="A Claw model cannot be selected until its provider is known.",
    ...
    if args:
        ...
        if runtime.config.active_backend == "claw-cli" and not available:
            mode_flag = runtime._claw_provider_mode(active=True)
            await runtime._reply_text(
                update,
...
async def callback_model(runtime, update, context: Any) -> None:
    ...
    try:
        if data == "backend_mode_confirm":
            ...
            provider = runtime.get_current_provider()
            available = runtime._get_available_models()
            if runtime.config.active_backend == "claw-cli" and provider and not available:
                mode_flag = runtime._claw_provider_mode(active=True)
                await query.edit_message_text(
                    runtime._build_claw_provider_model_text(provider, mode_flag),
                    parse_mode="HTML",
            ...
            parts = data.split(":", 3)
            if len(parts) != 4:
                await query.answer("Invalid Claw model selection.", show_alert=True)
                return
            ...
            if not model:
                await query.answer(
                    "This Claw model is no longer available.",
                    show_alert=True,
                )
                return
