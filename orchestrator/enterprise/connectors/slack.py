...
SlackTransport = Callable[[str, Mapping[str, Any]], Mapping[str, Any]]
...
class SlackWebhookConnector:
    ...
    def health_check(self) -> ConnectorHealth:
        if not self.webhook_url:
            return ConnectorHealth(ok=False, status="unhealthy", message="Slack webhook URL is not configured")
        return ConnectorHealth(ok=True, status="healthy", message="Slack webhook connector configured")
    ...
    def execute(self, action: ConnectorAction) -> ConnectorResult:
        ...
        return ConnectorResult(
            ...
            message=f"unsupported Slack connector action: {action.action}",
    ...
    def _send_message(self, action: ConnectorAction) -> ConnectorResult:
        ...
        if action.dry_run:
            return ConnectorResult(ok=True, status="dry_run", message="Slack message dry run", data={"payload": payload})
        response = self._post(payload)
        return ConnectorResult(ok=True, status="success", message="Slack message sent", data=dict(response))
