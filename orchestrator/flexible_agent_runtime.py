...
class FlexibleAgentRuntime:
    ...
    async def cmd_loop(self, update: Update, context: Any):
        ...
        # --- /loop <task> — typed scheduler setup ---
        loop_skill_prompt = (
            "--- SKILL CONTEXT [loop] ---\n"
            "The user wants a recurring task. Understand the request and create it "
            "through the typed `hashi_scheduler_create` tool; never read or edit "
            "tasks.json directly.\n\n"
            "Use a heartbeat for interval-based loops (every N minutes/hours, polling, "
            "retries, watchdogs). Use a cron only for fixed wall-clock times. Include "
            "a clear prompt, a concise note, and a loop_max when the user gives a cap "
            "or a safe default is needed. If the interval is unclear, use 10 minutes.\n"
            "After the tool succeeds, tell the user the task id, kind, schedule, and "
            "what each iteration will do. The Scheduler owns persistence and identity.\n\n"
            "--- USER REQUEST ---\n"
            f"{args_text}"
        )
    ...
    async def cmd_hchat(self, update: Update, context: Any):
        ...
        if broadcast_targets is not None:
            ...
            self_prompt = (
                ...
                f"Use 'sent' only for a confirmed Frontend Connector transport receipt; "
                f"protocol lifecycle states remain queued, and any failed target is failed.\n\n"
