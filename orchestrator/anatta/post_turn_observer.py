...
class AnattaPostTurnObserver(PostTurnObserver, PreTurnContextProvider):
    ...
    def should_observe(self, source: str, *, is_bridge_request: bool) -> bool:
        if self._should_bypass_source(source, is_bridge_request=is_bridge_request):
            return False
        mode = self._mode()
    ...
    def _should_bypass_source(cls, source: str, *, is_bridge_request: bool) -> bool:
        ...
        if not normalized:
            return True
        if normalized in cls.BYPASS_SOURCES:
            return True
        return normalized.startswith(("bridge:", "bridge-transfer:", "hchat-reply:", "ticket:", "cos-query:"))
