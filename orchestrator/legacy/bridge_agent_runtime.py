...
class BridgeAgentRuntime:
        ...
        if not args:
            await update.message.reply_text(
                setting_card(
                    "🫧",
                    "Active continuation",
                    current=html.escape(self.skill_manager.describe_active_heartbeat(self.name)),
                    facts=[f"<b>Agent</b> · <code>{html.escape(self.name)}</code>"],
                    consequence="Changes take effect immediately and persist in the task manager.",
                    action="Use <code>/active on [minutes]</code> or <code>/active off</code>.",
                ),
                parse_mode="HTML",
            )
            return
    ...
    async def cmd_skill(self, update, context):
        ...
        if sub in {"cron", "heartbeat"}:
            ...
            return
        if sub == "dream":
            await update.message.reply_text("Dream is a runtime command. Use /dream.")
            return
        if sub == "recall":
            rest = " ".join(args[1:]).strip().lower()
            if rest not in {"on", "off"}:
                await update.message.reply_text(
                    "Recall is a runtime setting. Usage: /skill recall on|off"
                )
                return
    ...
    async def callback_skill(self, update, context):
        ...
        if data.startswith("skilljob:"):
            ...
            if action == "toggle":
                ...
                await self._render_jobs(query, kind)
                return
            if action == "delete":
    ...
    def _model_keyboard(self) -> InlineKeyboardMarkup:
        ...
        buttons = []
        for m in models:
            label = selected_label(m, m == self.config.model)
    ...
    def _effort_keyboard(self) -> InlineKeyboardMarkup:
        ...
        for level in self._get_available_efforts():
            label = selected_label(level, level == current)
    ...
    async def cmd_effort(self, update, context):
        ...
        await update.message.reply_text(
            setting_card(
                "🎛️",
                "Model effort",
                current=f"<code>{html.escape(self._get_current_effort())}</code>",
                facts=[f"<b>Model</b> · <code>{html.escape(self.config.model)}</code>"],
