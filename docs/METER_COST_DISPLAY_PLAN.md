...
> 源根后，操作 `1d642c65ccac420fa67ed19eb93e67e5` 成功。Core PID `3216`
> 未变，agent1 Worker `31800` 已采用生成代 `sha256:83c4b09c…`；未执行 `/restart`。
...
`get_summary` / `get_summary_extended` 的持久与返回契约保持不变：
`cost_usd` 是已知记录的数值小计，`unknown_cost_requests` 是缺少成本的
请求数。用户界面必须同时解释这两个字段，不能把已知小计冒充完整总价：

- `unknown_cost_requests == 0`：显示准确金额；真实 provider 零成本和本地零
  成本继续显示 `$0.0000`。
- 所有请求成本未知：显示本地化的“成本未知”，不显示 `$0.0000`。
- 已知与未知请求并存：显示本地化的“已知小计”，并注明未知请求数。

统一格式器由 `/usage`、`/usage all`、系统 `/token` 汇总和 `/status` 用量行
共同投影；跨 Agent/Backend/时间段聚合必须同时累计
`unknown_cost_requests`。这项修复不修改价目表、计费算法、历史账本、模型
选择或自动价格获取，也不新增状态写入器。
...
- 缓存键严格区分实际执行 Engine、配置模型和来源模型。已限定 ID 直接使用；
  ...
  `data.id` 必须与来源 ID 完全一致，不做 family、prefix 或 fuzzy 猜测。
- `https://openrouter.ai/openapi.json` 是币种与计价单位契约来源；其
  ...
  版本。
- 保存 prompt、completion、cache read/write、独立 reasoning、request 和来源
  提供的其他维度。`min_prompt_tokens` 条件阶梯按每次物理调用选择，未知条件
  fail closed；只有本次用量涉及的维度都有费率才估值。全零只有明确的
  `:free` route 才能标为 `known_zero`。
- Provider 实报金额（包括真实 `0.0`）始终优先；明确本地模型其次。OpenRouter
  路由可使用其精确目录估值；Codex／DeepSeek／xAI／Claude／Gemini 等其他执行
  渠道在没有实报金额时可使用同模型事实，并在金额前保留 `≈` 表示估值；具体
  `cost_source` 与价格 revision 继续保存在用量凭据中，不在紧凑回合尾注明来源，
...
使用共享安全 JSON 写入器、进程锁、原子替换、单模型 64 KiB／目录 8 MiB
流式响应硬上限、HTTPS 域名白名单、5 秒连接
