...
Release focus: **HER mode foundation + Superloop operational foundation**.
...
- HASHI can discover a packaged HER runtime through manifest metadata and
  checksum validation.
...
- `adapters/her.py`
- backend registry entry for `her`
...
- packaging docs under `packaging/her/`
- provider smoke probe:
  `scripts/her_runtime_probe.py`

Validated:

- focused HER adapter tests pass;
...
- momo live integration validated repo-root read/write/edit through HER mode.
...
- Packaged `hashi-her` binaries are not yet shipped under
  `hashi_assets/her/bin/`.
- `scripts/her_runtime_probe.py --check version` may correctly return
...
```text
python -m py_compile setup.py adapters/her.py tests/test_her_adapter.py
