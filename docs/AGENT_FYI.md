...
- `/effort [level]`: available when the active backend supports effort levels. Grok CLI offers `low`, `medium`, and `high` with a HASHI default of `medium`. Codex choices follow the active model. HER v2 instead shows **HER execution mode**: Direct (`zero`), Fast path (`low`), Planned (`medium`), Adaptive (`high`), Reviewed (`xhigh`), and Assured (`max`). Direct is one fully capable Quick-model call with default provider reasoning `high`; it never upgrades and has no Immediate, Triage, Planning, Replanning, delegation, Review, Verification, or Finalisation stage. Its natural-language return is `COMPLETED`, even when it asks for missing information. Planned fixes the measured stage boundary as no-tool Strategy, mechanically read-only Planning, then fully capable Execution; Planning may investigate but cannot mutate artifacts or perform the downstream implementation. Normal `/verbose` provider/tool progress and the existing local attachment fallback remain available. Descriptive aliases normalize to the canonical wire values. Reviewed adds tool-backed read-only Review and closure; Assured adds latest-state Verification with at most three attempts. HER effort does not change provider reasoning configured through `/model`.
