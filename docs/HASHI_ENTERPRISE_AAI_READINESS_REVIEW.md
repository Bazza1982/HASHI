...
- A production hardening runbook and Helm values example cover TLS ingress, NetworkPolicy, HPA, PDB, resource sizing, render review, rollout checks, and rollback guidance.
- Helm render CI now covers the production hardening values and verifies that Ingress, NetworkPolicy, HPA, PDB, TLS, and ingress namespace selector resources render.
- A production validation command-plan generator emits the kubectl/curl checklist for ingress, NetworkPolicy, HPA, PDB, HTTPS health, and rollback review.
- A GitHub Actions workflow publishes the production validation command plan as an artifact without running Helm, kubectl, or curl.
...
- Workbench Connector Test Run renders schema-driven controls for scalar and enum parameters while keeping the raw JSON editor for complex parameters.
