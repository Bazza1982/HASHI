...
- OIDC authorization-code start plus callback state/code validation before token exchange;
...
- OIDC start returns an authorization URL while keeping the PKCE `code_verifier` server-side; callback validation consumes state and avoids writing authorization codes into audit.
