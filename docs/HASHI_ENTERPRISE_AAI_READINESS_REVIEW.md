...
- SCIM-style provisioning primitives and admin-gated HTTP handlers can create, update, deactivate, and reactivate users, assign default project membership, and revoke sessions/API tokens on deactivation.
...
- complete SAML login flow with XML Signature verification and standards-compatible SCIM 2.0 HTTP flows, including groups, filters, pagination, and IdP schema negotiation;
