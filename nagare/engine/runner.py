...
from nagare.yaml.codec import load_workflow_file
...
class FlowRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self.state = TaskState(self.run_id, runs_root=self.runs_root)
        self.state.set_workflow_metadata(
        ...
        self.dispatcher = self.step_handler
        # 信号文件路径（外部进程可通过创建这些文件来控制工作流）
        self._run_dir = self.runs_root / self.run_id
        ...
        self.event_logger.emit(
            ...
            component="engine.runner",
            message="Workflow definition loaded",
            data={
                "workflow_id": workflow_meta.get("id"),
                "workflow_version": workflow_meta.get("version"),
            },
    ...
    def start(self) -> dict:
        ...
        self.event_logger.emit(
            ...
            data={"workflow_id": self.workflow["workflow"]["id"]},
        )
    ...
    def pause(self, reason: str = ""):
        ...
        self._emit_event("run.paused", "Pause requested", {"reason": reason or None})
    ...
    def resume(self):
        ...
        self._emit_event("run.resumed", "Pause cleared", {})
    ...
    def abort(self, reason: str = ""):
        ...
        self._emit_event("run.cancelled", "Stop requested", {"reason": reason or None})
    ...
    def _check_signals(self) -> str:
        ...
        if self._pause_signal.exists():
            ...
            self._emit_event("run.resumed", "Workflow resumed after pause", {})
    ...
    def _extract_questions(self, step: dict, result: dict) -> list:
        ...
        for key, path in artifacts.items():
            try:
                full_path = self.runs_root / self.run_id / "workers" / step["agent"] / path
                if not full_path.exists():
                    full_path = self.runs_root / self.run_id / "workers" / path
                if not full_path.exists():
                    full_path = Path(path)
    ...
    def _execute_step(self, step: dict) -> dict:
        ...
        try:
            ...
            if result["status"] == "completed":
                ...
                error_type = result.get("error_type", "unknown")
                self.state.set_step_status(step_id, StepStatus.FAILED, error=error_msg)
                self._emit_event(
                    "step.failed",
            ...
            duration = time.time() - start_time
            self.state.set_step_status(step_id, StepStatus.FAILED, error=str(e))
            self._emit_event(
                "step.failed",
    ...
    def _handle_failure(self, step: dict, failure_result: dict) -> bool:
        ...
        self._emit_event(
            ...
            {"step_id": step["id"], "debug_agent": debug_agent},
            step_id=step["id"],
        )
        ...
        self._emit_event(
            ...
            step_id=step["id"],
            error_code=None if recovered else result.get("error_type"),
            error_message=None if recovered else result.get("error_message"),
        )
    ...
    def _send_task_via_hchat(self, agent_id: str, message: dict) -> dict:
        ...
        result = self.step_handler.execute(
            agent_id=agent_id,
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        logger = logging.getLogger(f"nagare.runner.{self.run_id}")
        logger.setLevel(logging.DEBUG)
        if not any(
            isinstance(handler, logging.FileHandler)
            and getattr(handler, "baseFilename", "").endswith("flow_runner.log")
