...
from tools.browser_bridge_smoke_runner import execute_smoke_plan, write_smoke_command_plan
...
def run_option_d_isolated_acceptance(
    root_dir: Path,
    *,
    source_extension_dir: Path,
    chrome_exe: str,
    ...
    materialized = materialize_option_d_test_harness(
        root_dir,
        source_extension_dir=source_extension_dir,
        chrome_exe=chrome_exe,
        windows_user_data_dir=windows_user_data_dir,
        windows_extension_dir=windows_extension_dir,
        windows_native_host_manifest_path=windows_native_host_manifest_path,
        windows_host_command_path=windows_host_command_path,
        repo_root=repo_root,
        distro_name=distro_name,
        socket_path=socket_path,
        log_path=log_path,
    ...
    smoke_results = execute_smoke_plan(
        root_dir,
        repo_root=Path(repo_root),
        runner=smoke_runner if smoke_runner is not None else __import__("subprocess").run,
    )
    acceptance_summary = write_acceptance_summary(root_dir)
    return {
