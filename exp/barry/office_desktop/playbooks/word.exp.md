...
1. Use a template or object-level document construction for consistent layout.
2. Keep report hierarchy explicit with title, headings, summary, body sections,
   tables, and footer.
3. For real UI validation, open the file in Word, capture the editing view, and
   verify content is visible.
4. For PDF output, use File > Export > Create PDF/XPS rather than Save As with a
   `.pdf` filename.
5. Validate the saved `.docx` structurally when possible: headings, table XML,
